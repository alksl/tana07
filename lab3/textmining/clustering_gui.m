...
try, load(strcat(datasets{id}, '\titles.mat')); catch, titles=cell(size(A, 2), 1); end
...
if get(handles.StoreRes, 'Value')==1, 
    ...
    if get(handles.kmeansRadio, 'Value')==1, 
        str=strcat(dataset, '\kmeans');
        if ~exist(str), mkdir(str); end
        str=strcat(str, '\k_', num2str(nc));
        if ~exist(str), 
            mkdir(str); 
        else, 
            str1=strcat(str, '\clusters_kmeans.mat');
            if exist(str1), delete(str1); end; 
        end
        str1=strcat(str, '\clusters_kmeans');
        save(str1, 'clusters_kmeans'); 
    end
    if get(handles.skmeansRadio, 'Value')==1, 
        str=strcat(dataset, '\skmeans');
        if ~exist(str), mkdir(str); end
        str=strcat(str, '\k_', num2str(nc));
        if ~exist(str), 
            mkdir(str); 
        else, 
            str1=strcat(str, '\clusters_skmeans.mat');
            if exist(str1), delete(str1); end; 
        end
        str1=strcat(str, '\clusters_skmeans');
        save(str1, 'clusters_skmeans'); 
    end    
    if get(handles.PDDPRadio, 'Value')==1, 
        str=strcat(dataset, '\pddp');
        if ~exist(str), mkdir(str); end
        str=strcat(str, '\k_', num2str(nc));
        if ~exist(str), 
            mkdir(str); 
        else, 
            str1=strcat(str, '\clusters_pddp.mat');
            if exist(str1), delete(str1); end; 
        end
        str1=strcat(str, '\clusters_pddp');
