...
if get(handles.StoredLabels, 'Value')==1, 
    try, 
        load(strcat(str, '\labels.mat')); 
    catch, 
        an=sprintf('Variable %s not found...', strcat(str, '\labels.mat'));
        msgbox(an, 'Error', 'modal');
        return;             
    end
else, 
...
guidata(hObject, handles);
try, 
	load(strcat(str, '\dictionary.mat'));
catch, 
	an=sprintf('Dictionary not found in %s...', strcat(str, '\dictionary.mat'));
	msgbox(an, 'Error', 'modal');
	return;             
end        
if get(handles.FQueryRadio, 'Value')==1, 
